...
// For Vercel deployment, we need to handle database differently
...
class Database {
    ...
    async initialize() {
        return new Promise((resolve, reject) => {
            if (this.isVercel) {
                // In Vercel, use in-memory database with backup to /tmp
                this.db = new sqlite3.Database(DB_PATH, (err) => {
                    if (err) {
                        console.error('Erro ao conectar com o banco de dados:', err);
                        reject(err);
                    } else {
                        console.log('Conectado ao banco de dados SQLite (Vercel)');
